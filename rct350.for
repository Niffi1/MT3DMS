C
      SUBROUTINE RCT3AL(INRCT,IOUT,ISUM,ISUM2,NCOL,NROW,NLAY,NCOMP,
...
C--PRINT PACKAGE NAME AND VERSION NUMBER
 ...
 1000 FORMAT(1X,'RCT3 -- CHEMICAL REACTION PACKAGE,',
...
C
C
      SUBROUTINE RCT3RP(IN,IOUT,NCOL,NROW,NLAY,NCOMP,ICBUND,COLD,
...
C--OR IMMOBILE-LIQUID PHASE IN DUAL-DOMAIN MODEL (ISOTHM=5 OR 6)
      ...
      ENDIF
C
C--CALL [SRCT3R] TO CALCULATE RETARDATION FACTORS FOR BOTH DOMAINS
...
C--OR IMMOBILE-LIQUID PHASE CONCENTRATION (DUAL-DOMAIN MODEL)
      IF(ISOTHM.GT.0) THEN
        ...
        TR=0.
        DO INDEX=1,NCOMP
          CALL SRCT3R(NCOL,NROW,NLAY,ICBUND(1,1,1,INDEX),PRSITY,
...
C
C
      SUBROUTINE RCT3SV(NCOL,NROW,NLAY,NCOMP,ICOMP,ICBUND,PRSITY,
...
C--RECORD SORBED MASS STORAGE CHANGE
              IF(DCRCT.LT.0) THEN
                RMASIO(20,2,ICOMP)=RMASIO(20,2,ICOMP)+DCRCT*
     &              DELR(J)*DELC(I)*DH(J,I,K)*PRSITY(J,I,K)
              ELSE
                RMASIO(20,1,ICOMP)=RMASIO(20,1,ICOMP)+DCRCT*
     &              DELR(J)*DELC(I)*DH(J,I,K)*PRSITY(J,I,K)
              ENDIF
            ENDDO
          ENDDO
...
C--RECORD MASS STORAGE CHANGE IN IMMOBILE DOMAIN
              IF(DCRCT.LT.0) THEN
                RMASIO(21,2,ICOMP)=RMASIO(21,2,ICOMP)
     &           +DCRCT*PRSITY(J,I,K)*RETA(J,I,K,ICOMP)
     &           *DELR(J)*DELC(I)*DH(J,I,K)
     &           /RETA2(J,I,K,ICOMP)
                RMASIO(22,2,ICOMP)=RMASIO(22,2,ICOMP)
     &           +DCRCT*PRSITY(J,I,K)*RETA(J,I,K,ICOMP)
     &           *DELR(J)*DELC(I)*DH(J,I,K)
     &           *(RETA2(J,I,K,ICOMP)-1.)/RETA2(J,I,K,ICOMP)
              ELSE
                RMASIO(21,1,ICOMP)=RMASIO(21,1,ICOMP)
     &           +DCRCT*PRSITY(J,I,K)*RETA(J,I,K,ICOMP)
     &           *DELR(J)*DELC(I)*DH(J,I,K)
     &           /RETA2(J,I,K,ICOMP)
                RMASIO(22,1,ICOMP)=RMASIO(22,1,ICOMP)
     &           +DCRCT*PRSITY(J,I,K)*RETA(J,I,K,ICOMP)
     &           *DELR(J)*DELC(I)*DH(J,I,K)
     &           *(RETA2(J,I,K,ICOMP)-1.)/RETA2(J,I,K,ICOMP)
              ENDIF
...
C--RECORD SORBED MASS STORAGE CHANGE FOR NONEQUILIBRIUM SORPTION
            IF(ISOTHM.EQ.4.AND.DCRCT2.GT.0) THEN
              RMASIO(20,2,ICOMP)=RMASIO(20,2,ICOMP)-DCRCT2
     &        *PRSITY(J,I,K)*RETA(J,I,K,ICOMP)
     &        *DELR(J)*DELC(I)*DH(J,I,K)
            ELSEIF(ISOTHM.EQ.4.AND.DCRCT2.LT.0) THEN
              RMASIO(20,1,ICOMP)=RMASIO(20,1,ICOMP)-DCRCT2
     &        *PRSITY(J,I,K)*RETA(J,I,K,ICOMP)
     &        *DELR(J)*DELC(I)*DH(J,I,K)
            ENDIF
          ENDDO
...
C--RECORD MASS STORAGE CHANGE IN IMMOBILE DOMAIN
            IF(CIML.GT.0) THEN
              RMASIO(21,2,ICOMP)=RMASIO(21,2,ICOMP)-CIML
            ELSE
              RMASIO(21,1,ICOMP)=RMASIO(21,1,ICOMP)-CIML
            ENDIF
            IF(CIMS.GT.0) THEN
              RMASIO(22,2,ICOMP)=RMASIO(22,2,ICOMP)-CIMS
            ELSE
              RMASIO(22,1,ICOMP)=RMASIO(22,1,ICOMP)-CIMS
            ENDIF
          ENDDO
        ENDDO
      ENDDO
...
C
C
      SUBROUTINE SRCT3R(NCOL,NROW,NLAY,ICBUND,PRSITY,COLD,RETA,RFMIN,
...
C
C
      SUBROUTINE RCT3FM(NCOL,NROW,NLAY,NCOMP,ICOMP,ICBUND,PRSITY,
...
C
C
      SUBROUTINE RCT3BD(NCOL,NROW,NLAY,NCOMP,ICOMP,ICBUND,PRSITY,
...
C--RECORD MASS STORAGE CHANGE IN IMMOBILE DOMAIN
              IF(DCRCT.LT.0) THEN
                RMASIO(21,2,ICOMP)=RMASIO(21,2,ICOMP)+DCRCT
     &          /RETA2(J,I,K,ICOMP)
                RMASIO(22,2,ICOMP)=RMASIO(22,2,ICOMP)+DCRCT
     &          *(RETA2(J,I,K,ICOMP)-1.)/RETA2(J,I,K,ICOMP)
              ELSE
                RMASIO(21,1,ICOMP)=RMASIO(21,1,ICOMP)+DCRCT
     &          /RETA2(J,I,K,ICOMP)
                RMASIO(22,1,ICOMP)=RMASIO(22,1,ICOMP)+DCRCT
     &          *(RETA2(J,I,K,ICOMP)-1.)/RETA2(J,I,K,ICOMP)
              ENDIF
            ENDDO
          ENDDO
...
C
C
      SUBROUTINE RCT3CF(NCOL,NROW,NLAY,NCOMP,ICOMP,ICBUND,PRSITY,
