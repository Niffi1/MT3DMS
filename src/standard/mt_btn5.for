...
C
C
      SUBROUTINE BTN5AD(NTRANS,MXTRNOP,iUnitTRNOP,TIME1,TIME2,HT2,DELT,
...
C--DETERMINE STEPSIZE FOR NEXT TRANSPORT STEP
      IF(IMPSOL.EQ.0) THEN
        IF(MIXELM.GT.0) THEN
          ...
          IF(iUnitTRNOP(2).GT.0) DTRANS=MIN(DTRANS,DTDISP*RFMIN)
          IF(iUnitTRNOP(3).GT.0) DTRANS=MIN(DTRANS,DTSSM*RFMIN)
          IF(iUnitTRNOP(4).GT.0) DTRANS=MIN(DTRANS,DTRCT)
        ELSE
          DTRANS=0.
          IF(iUnitTRNOP(1).GT.0) DTRANS=DTRANS+1./(DTRACK2*PERCEL*RFMIN)
          IF(iUnitTRNOP(2).GT.0) DTRANS=DTRANS+1./(DTDISP*RFMIN)
          IF(iUnitTRNOP(3).GT.0) DTRANS=DTRANS+1./(DTSSM*RFMIN)
          IF(iUnitTRNOP(4).GT.0) DTRANS=DTRANS+1./DTRCT
          DTRANS=1./DTRANS
        ENDIF
