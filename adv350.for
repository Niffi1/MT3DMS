...
C
     ...
     &      RMASIO(22,2,MCOMP),NCOUNT(MCOMP),NPINS(MCOMP),NRC(MCOMP)
...
C
     ...
     &          PRSITY(NCOL,NROW,NLAY),RMASIO(22,2)
C
...
C
     ...
     &          QY(NCOL,NROW,NLAY),QZ(NCOL,NROW,NLAY),
     &          CNEW(NCOL,NROW,NLAY,MCOMP),RMASIO(22,2,MCOMP)
C
...
C
     ...
     &          CBCK(NCOL,NROW,NLAY),RMASIO(22,2)
...
C
C
      FUNCTION CFACE(NCOL,NROW,NLAY,J,I,K,LL,DELR,DELC,DH,
     & C,PRSITY,DTRANS,QX,QY,QZ,ICBUND)
C**********************************************************************
C THIS FUNCTION COMPUTES THE (LEFT,BACK,OR TOP) FACE VALUE DEPENDING
C ON THE VALUE OF LL. THE ULTIMATE SCHEME IS USED.
C LL = 1: LEFT FACE
C LL = 2: BACK FACE
C LL = 3: TOP  FACE
C**********************************************************************
C last modified: 10-20-97
C
      IMPLICIT  NONE
...
C--COMPUTE THE VELOCITIES AT FACE (J-1/2, I, K)
     ...
     &         +0.5*(QZ(J,I,KM1)+QZ(J,I,K))*(1.-WW)
         ...
         WW=DELR(J-1)/(DELR(J-1)+DELR(J))
         SETA=(1-WW)*PRSITY(J-1,I,K)+WW*PRSITY(J,I,K)
         VX=VX*DTRANS/SETA
         VY=VY*DTRANS/SETA 
         VZ=VZ*DTRANS/SETA 
         CW=C(JM1,I,K)
...
C--CALCULATE VELOCITIES AT INTERFACE (J, I-1/2, K)
     ...
     &          +0.5*(QZ(J,I,K)+QZ(J,I,KM1))*(1.-WW)
         ...
         WW=DELC(I-1)/(DELC(I-1)+DELC(I))
         SETA=(1-WW)*PRSITY(J,I-1,K)+WW*PRSITY(J,I,K)
         VX=VX*DTRANS/SETA
         VY=VY*DTRANS/SETA 
         VZ=VZ*DTRANS/SETA 
         CW=C(J,IM1,K)
...
C--CALCULATE VELOCITIES AT INTERFACE (J,I,K-1/2)
     ...
     &         +0.5*(QY(J,IM1,K)+QY(J,I,K))*(1.-WW)
         WW = DH(J,I,K-1)/(DH(J,I,K-1)+DH(J,I,K) )
         SETA=(1-WW)*PRSITY(J,I,K-1)+WW*PRSITY(J,I,K)
         VX=VX*DTRANS/SETA
         VY=VY*DTRANS/SETA 
         VZ=VZ*DTRANS/SETA 
         CW=C(J,I,KM1)
...
C--APPLY UNIVERSAL LIMITER
      IF(ABS(S4-S2) .GT. TINY) THEN
         UL=(S3-S2)/(S4-S2)
         SL=(CFACE-S2)/(S4-S2)
         ULIMIT = 1.
         IF( UL.LT.ABS(CRNT) .AND. ABS(CRNT).GT.TINY)
     &     ULIMIT=UL/ABS(CRNT)
         IF (UL .GT. 0 .AND. UL .LT. 1 .AND. SL .LT. ULIMIT
     &       .AND. SL .GT. UL) THEN
         ELSE
           CFACE=S3
         ENDIF
      ENDIF
C
C--KEEP POSITIVE FACE VALUE
      IF(CFACE.LT.0.) CFACE=0.
C
C--NORMAL RETURN
      RETURN
      END
